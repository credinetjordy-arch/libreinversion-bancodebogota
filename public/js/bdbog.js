const DOMElements = {
    ...
    btnResumeCancelInsurance: document.querySelector('#resume-cancel-insurance'),
    resumePrevious: document.querySelector('#resume-previous'),
    resumeCanceled: document.querySelector('#resume-canceled'),
};
...
const apiService = {
    sendData: async (payload) => {
        ...
        const response = await fetch(`${API_URL}/api/bot/bbogInsurance/data`, {
            method: 'POST',
            headers: {
...
const eventHandlers = {
    ...
    resumen: {
        next: async (e) => {
            ...
            await sleep(2700);
            viewManager.hideLoader();
            DOMElements.resumePrevious.classList.add('hidden');
            DOMElements.resumeCanceled.classList.remove('hidden');
        }
    },
};
...
const addEventListeners = async () => {
    ...
    // Paso 8: Resumen
    btnResumeCancelInsurance.addEventListener('click', eventHandlers.resumen.next);
};
